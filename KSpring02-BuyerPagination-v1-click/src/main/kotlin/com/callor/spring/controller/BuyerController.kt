...
//    @ResponseBody
    ...
    fun list(model: Model, pageable: Pageable): String {
        ...
        // pageNumber 이거 위에 넘겨준 BUYERS를 받아 html에서 BUYERS.pageNumber로 사용하면된다.
        // 굳이 여기서 분리해서 넘겨줄 필요 없음
...
//        return buyerList
    ...
    fun list(model: Model, @PathVariable("page") page:String="0"): String {
        val intPage = try{
            ...
            0 // return 쓰는 거 아니다. 오류나면 0으로 만들어라.
        }
        logger().debug("여기는 page list 함수")
//        val buyerList = bService.selectAll(intPage)
        // 페이지네이션 구현을 위해 변경
